...
$(document).ready(function() {
	...
	function updateComp() {
		...
		$.ajax({
			...
			success: function(result) {
				if(result.res == "SUCCESS") {
					makeAlert(2, "수정 성공", "성공적으로 수정했습니다.", function() {
						$("#dataForm").attr("action", "compDetail");
						$("#dataForm").submit();
					});
				}
				else {
					makeAlert(2, "수정 실패", "수정에 실패했습니다.", null);
				}
			},
			error : function(request, status, error) {
				console.log("status : " + request.status);
...
<body>
	...
	<div class="title_area no_drag">고객사 수정</div>
					...
					<tbody>	
                		<tr>
                    		...
                    		<td class="field_contents">
                        		<input type="text" class="input_normal" id="comp_name" name="comp_name" value="${data.COMP_NAME}"/>
                    		</td>
                    		<td class="field_name no_drag">담당자 <span class="acc_txt">*</span></td>
                    		<td class="field_contents">
                        		<input type="text" class="input_normal input_readonly no_drag" id="emp_name" style="width: calc(100% - 100px) !important" readonly="readonly" value="${data.EMP_NAME}"/>
                		...
                		<tr>
                    		...
                    		<td class="field_contents">
                        		<input type="text" class="input_normal" id="comp_ph" name="comp_ph" placeholder="'-' 포함" value="${data.COMP_PH}" />
                    		</td>
                    		...
                    		<td class="field_contents">
                        		<select class="input_normal no_drag" style="width: calc(100% - 35px) !important;" id="comp_grade" name="comp_grade">
                            		<option value="0_#ffffff">- - 선택 - -</option>
                            		<c:forEach var="grade" items="${grade}">
                            			<c:choose>
                            				<c:when test="${grade.G_NO eq data.CC_GRADE_NO}"><option value="${grade.G_NO}_${grade.G_COLOR}" selected="selected">${grade.G_NAME}</option></c:when>
                            				<c:otherwise><option value="${grade.G_NO}_${grade.G_COLOR}">${grade.G_NAME}</option></c:otherwise>
                            			</c:choose>
                            		</c:forEach>
                        		</select>
                        		<div class="color_view no_drag" style="background-color: ${data.GRADE_COLOR}"></div>
                    		</td>
                		</tr>
                		<tr>
                    		<td class="field_name first_field_name no_drag">업종 <span class="acc_txt">*</span></td>
                    		<td class="field_contents">
                        		<input type="text" class="input_normal input_readonly" id="comp_sect" style="width: calc(100% - 100px) !important" readonly="readonly" value="${data.SECT_NAME}" />
                        		<div class="btn btn_black btn_size_normal no_drag" id="sectSearchBtn">검색</div>
                        		<input type="hidden" id="sect_no" name="sect_no" value="${data.COMP_SECT_NO}" />
                    		</td>
                    		...
                    		<td class="field_contents">
                        		<select class="input_normal no_drag" id="comp_stat" name="comp_stat">
                            		...
                            		<c:forEach var="stat" items="${stat}">
                            			<c:choose>
                            				<c:when test="${stat.STAT_NO eq data.COMP_STAT_NO}"><option value="${stat.STAT_NO}" selected="selected">${stat.STAT_NAME}</option></c:when>
                            				<c:otherwise><option value="${stat.STAT_NO}">${stat.STAT_NAME}</option></c:otherwise>
                            			</c:choose>
                            		</c:forEach>	
                        		</select>
                		...
                		<tr>
                    		...
                    		<td class="field_contents" colspan="3">
                        		<input type="text" class="input_normal" id="comp_web" name="comp_web" value="${data.COMP_WEBSITE}" />
                    		</td>
                		</tr>
                		<tr>
                    		...
                    		<td class="field_contents" colspan="3">
                        		<div class="address_margin">
                            		<input type="text" class="input_short input_readonly postcodify_postcode5" readonly="readonly" placeholder="우편번호" id="comp_zc" name="comp_zc" value="${data.ZC}"/>
                            		<div class="btn btn_black btn_size_normal no_drag" id="searchAddrBtn">검색</div>
                        		</div>
                        		<div class="address_margin">
                            		<input type="text" class="input_normal input_readonly postcodify_address" readonly="readonly" placeholder="주소" id="comp_addr" name="comp_addr" value="${data.ADDR}"/>
                        		</div>
                        		<div><input type="text" class="input_normal postcodify_details" placeholder="상세주소" id="comp_det_addr" name="comp_det_addr" value="${data.DETAIL_ADDR}" /></div>
                    		</td>
                		</tr>
                		<tr>
                    		<td class="field_name first_field_name no_drag">상세정보</td>
                    		<td class="field_contents" colspan="3">
                        		<textarea type="text" class="textarea_normal" id="comp_det_info" name="comp_det_info">${data.DETAIL_INFO}</textarea>
                    		</td>
                		</tr>
					</tbody>
            	</table>
